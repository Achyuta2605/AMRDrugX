...
def start_dti_fargate_task(
    ...
    cluster_name = _get_required_env("ECS_CLUSTER_NAME")
    task_definition = _get_required_env("ECS_TASK_DEFINITION")
    container_name = _get_required_env("ECS_CONTAINER_NAME")
    ...
    container_environment = [
        ...
        {"name": "AMRDRUGX_SCREENING_BUCKET", "value": bucket_name},
        {"name": "JOB_ID", "value": job_id},
        {"name": "INPUT_KEY", "value": input_key},
        {"name": "OUTPUT_KEY", "value": output_key},
        {"name": "DTI_SCORER_BACKEND", "value": "deeppurpose"},
    ]

    deeppurpose_model_name = os.getenv("DEEPPURPOSE_MODEL_NAME")
    if deeppurpose_model_name:
        container_environment.append(
            {"name": "DEEPPURPOSE_MODEL_NAME", "value": deeppurpose_model_name}
        )
    ...
    if failures:
        raise RuntimeError(f"ECS failed to start Fargate task: {failures}")

    tasks = response.get("tasks", [])
    if not tasks:
        raise RuntimeError("ECS did not return a started task.")
