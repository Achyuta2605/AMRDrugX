...
REQUIRED_ENV_VARS = [
    ...
    "INTERACTION_ECS_CLUSTER_NAME",
    "INTERACTION_ECS_TASK_DEFINITION",
    "INTERACTION_ECS_CONTAINER_NAME",
    "ECS_SUBNET_IDS",
...
def start_interaction_fargate_task(
    job_id: str,
    input_key: str,
    output_key: str,
) -> Dict[str, Any]:
    ...
    cluster_name = _get_required_env("INTERACTION_ECS_CLUSTER_NAME")
    task_definition = _get_required_env("INTERACTION_ECS_TASK_DEFINITION")
    container_name = _get_required_env("INTERACTION_ECS_CONTAINER_NAME")
    ...
    if failures:
        raise RuntimeError(f"ECS failed to start interaction task: {failures}")

    tasks = response.get("tasks", [])
    if not tasks:
        raise RuntimeError("ECS did not return a started interaction task.")
